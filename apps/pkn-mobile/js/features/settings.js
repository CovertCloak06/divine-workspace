...
export function updateSettingsUI() {
    ...
    if (placeholderInput) placeholderInput.value = settings.placeholder || 'Lay it on me...';
...
/**
 * Save placeholder text setting
 * @param {string} value - Placeholder text
 */
export function savePlaceholder(value) {
    const settings = loadSettings();
    settings.placeholder = value || 'Lay it on me...';
    saveSettings(settings);

    // Update the actual textarea placeholder
    const messageInput = document.getElementById('messageInput');
    if (messageInput) {
        messageInput.placeholder = settings.placeholder;
    }

    showToast('Placeholder text updated', 2000, 'success');
}
