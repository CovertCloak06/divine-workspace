...
def find_missing_selectors(project_dir):
    ...
    html_files = list(project_path.glob('*.html')) + list(project_path.glob('**/*.html'))
        ...
        try:
            with open(js_file, 'r', encoding='utf-8') as f:
                for line_num, line in enumerate(f, 1):
                    ...
                    id_matches = re.findall(r'getElementById\(["\']([^"\']+)["\']\)', line)
                    ...
                    qs_id_matches = re.findall(r'querySelector(?:All)?\(["\']#([^"\']+)["\']\)', line)
                    ...
                    class_matches = re.findall(r'classList\.(?:add|remove|toggle|contains)\(["\']([^"\']+)["\']\)', line)
                    ...
                    # Find querySelector('.foo') and querySelectorAll('.foo')
                    qs_class_matches = re.findall(r'querySelector(?:All)?\(["\']\.([^"\']+)["\']\)', line)
    ...
    missing_ids = {id_name: locations for id_name, locations in used_ids.items()
                   if id_name not in defined_ids}

    missing_classes = {class_name: locations for class_name, locations in used_classes.items()
