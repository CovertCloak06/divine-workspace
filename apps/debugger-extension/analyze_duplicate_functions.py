...
def find_duplicate_functions(project_dir):
    ...
    for js_file in js_files:
        if "node_modules" in str(js_file) or ".min.js" in str(
            js_file
        ):  # Skip minified and dependencies
            continue
                        ...
                        if match:
                            ...
                            if func_name not in [
                                ...
                                functions[func_name].append(
                                    (str(js_file.relative_to(project_path)), line_num)
                                )
    ...
    duplicates = {
        name: locations for name, locations in functions.items() if len(locations) > 1
    }
