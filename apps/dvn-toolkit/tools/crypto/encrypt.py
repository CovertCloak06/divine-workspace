...
def main():
    ...
    print(f"\n{BOLD}{CYAN}╔════════════════════════════════════════════════════════════╗{RESET}")
    print(f"{BOLD}{CYAN}║              🔐 File Encryptor (AES-256)                   ║{RESET}")
    ...
    try:
        if args.decrypt:
            ...
            print(f"  {GREEN}✓ File decrypted successfully{RESET}")
        else:
            encrypt_file(args.file, output_file, password)
            print(f"  {GREEN}✓ File encrypted successfully{RESET}")

        # Show file sizes
        in_size = os.path.getsize(args.file)
        out_size = os.path.getsize(output_file)
        print(f"  {DIM}Size: {in_size:,} → {out_size:,} bytes{RESET}")
