...
def shadow_image_analyze(image_path: str) -> str:
    """
    Full image analysis - EXIF, GPS, hashes, and reverse search URLs.
...
def shadow_image_exif(image_path: str) -> str:
    ...
    Gets: camera model, software, dates, exposure settings, lens info.

    Args:
        image_path: Path to image file

    Returns:
        JSON with EXIF data and intel notes
    """
...
def shadow_image_gps(image_path: str) -> str:
    ...
    Returns latitude, longitude, and direct links to maps.

    Args:
        image_path: Path to image file

    Returns:
        JSON with GPS coordinates and map URLs
    """
...
def shadow_image_ocr(image_path: str) -> str:
    ...
    Also identifies patterns like emails, phones, URLs, social handles.
    ...
    Returns:
        JSON with extracted text and identified patterns
    """
...
def shadow_image_reverse_search(image_path: str) -> str:
    """
    Get reverse image search URLs for multiple engines.

    Returns URLs for: Google, TinEye, Yandex, Bing, Baidu.

    Args:
        image_path: Path to image file (for reference)

    Returns:
        JSON with search engine URLs and tips
    """
...
def shadow_image_compare(image1_path: str, image2_path: str) -> str:
    ...
    Detects if images are the same even if resized, cropped, or filtered.
...
def shadow_image_hash(image_path: str) -> str:
    """
    Calculate file and perceptual hashes for an image.

    Returns: MD5, SHA256, pHash, aHash, dHash.
    Useful for finding duplicate or modified images.

    Args:
        image_path: Path to image file

    Returns:
        JSON with all hash values
    """
...
def shadow_image_strip_metadata(image_path: str, output_path: str = None) -> str:
    ...
    Strips EXIF, GPS, camera info for privacy.

    Args:
        image_path: Path to original image
        output_path: Path for cleaned image (optional, auto-generated if not provided)

    Returns:
        JSON with paths to original and cleaned images
    """
...
TOOL_DESCRIPTIONS = {
    ...
    # Image reconnaissance
    "shadow_image_analyze": "Full image analysis (EXIF, GPS, hashes, reverse search)",
    "shadow_image_exif": "Extract EXIF metadata (camera, software, dates)",
    "shadow_image_gps": "Extract GPS coordinates from image",
    "shadow_image_ocr": "Extract text from image using OCR",
    "shadow_image_reverse_search": "Get reverse image search URLs",
    "shadow_image_compare": "Compare two images for similarity",
    "shadow_image_hash": "Calculate file and perceptual hashes",
    "shadow_image_strip_metadata": "Remove all metadata from image for privacy",
}
