...
class RAGMemory:
    ...
    def index_file(self, filepath: Path, collection_name: str = "code"):
        ...
        # Select collection
        collection = (
            self.code_collection if collection_name == "code" else self.docs_collection
        )
        ...
        if len(lines) > 500:
            for i in range(0, len(lines), 500):
                chunk = "\n".join(lines[i : i + 500])
                chunks.append({"content": chunk, "chunk_id": i // 500})
        else:
            chunks.append({"content": content, "chunk_id": 0})
        ...
        for chunk in chunks:
            ...
            try:
                collection.add(
                    documents=[chunk["content"]], metadatas=[metadata], ids=[doc_id]
                )
                indexed_count += 1
            except Exception as e:
        ...
        return {"success": True, "file": str(filepath), "chunks_indexed": indexed_count}

    def index_codebase(
        self, extensions: List[str] = [".py", ".js", ".html", ".css", ".sh", ".md"]
    ):
        """Index all code files in the project"""

        results = {"indexed": [], "errors": [], "skipped": []}

        for ext in extensions:
            for filepath in self.project_root.rglob(f"*{ext}"):
                # Skip hidden directories and build artifacts
                if any(part.startswith(".") for part in filepath.parts):
                    if ".chroma_db" not in str(filepath) and ".venv" not in str(
                        filepath
                    ):
                        continue
                ...
                if any(
                    ...
                    for skip in ["node_modules", "__pycache__", "build", ".git"]
                ):
                    continue

                result = self.index_file(
                    filepath, collection_name="docs" if ext == ".md" else "code"
                )
                ...
                if result.get("success"):
                    ...
                    results["errors"].append(
                        {"file": str(filepath), "error": result["error"]}
                    )
                else:
    ...
    def search_code(
        self, query: str, n_results: int = 5, file_type: Optional[str] = None
    ) -> Dict[str, Any]:
    ...
    def add_conversation_memory(
        self, session_id: str, user_message: str, agent_response: str, agent_type: str
    ):
...
def search_relevant_code(
    query: str, project_root: str = "/home/gh0st/pkn", n_results: int = 5
):
...
def search_relevant_docs(
    query: str, project_root: str = "/home/gh0st/pkn", n_results: int = 3
):
