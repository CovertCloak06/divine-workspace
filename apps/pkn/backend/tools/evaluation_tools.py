...
class AgentEvaluator:
    ...
    def _add_default_categories(self):
        ...
        categories = [
            ...
            (
                "code_review",
                "review,check,analyze,examine,audit",
                "Reviewing code quality",
            ),
            ("explanation", "explain,describe,how,what,why", "Explaining concepts"),
            (
                "research",
                "research,find,search,lookup,documentation",
                "Finding information",
            ),
    ...
    def _classify_task(self, task: str) -> str:
        ...
        cursor = self.conn.cursor()
        cursor.execute(
            "SELECT name, keywords FROM task_categories WHERE keywords != ''"
        )
            ...
            metrics["by_category"].append(
                {
                    ...
                    "count": count,
                    "success_rate": (row["successful"] / count * 100)
                    if count > 0
                    else 0,
    ...
    def get_weak_areas(
        self, agent_type: str, min_failures: int = 3
    ) -> List[Dict[str, Any]]:
    ...
    def compare_agents(
        self, metric: str = "success_rate", days: int = 30
    ) -> List[Dict[str, Any]]:
            ...
            comparisons.append(
                {
                    ...
                    "total_executions": total,
                    "success_rate": (row["successful"] / total * 100)
                    if total > 0
                    else 0,
                    "avg_duration_ms": row["avg_duration"] or 0,
                    "avg_user_rating": row["avg_rating"] or 0,
    ...
    def export_metrics(self, output_file: Optional[str] = None) -> str:
        ...
        if not output_file:
            output_file = (
                self.project_root / "memory" / f"metrics_export_{int(time.time())}.json"
            )
