...
def api_chat():
    try:
        ...
        if model_id == 'local':
            current_app.logger.debug('Normalized legacy modelId "local" to llamacpp:local')
        ...
        if model_id.startswith('ollama:'):
            ...
            try:
                ...
                for parts in candidate_chat_paths:
                    ...
                    try:
                        current_app.logger.debug('Attempting Ollama chat POST to %s (model=%s)', url, ollama_model)
                        resp = requests.post(
                            url,
                            json={
                                'model': ollama_model,
                        ...
                        current_app.logger.debug('Ollama response from %s: status=%s', url, getattr(resp, 'status_code', None))
            ...
            try:
                ...
                current_app.logger.debug('Forwarding to local LLM at %s payload model=%s', url, payload.get('model'))
                resp = requests.post(url, json=payload, timeout=600)
                current_app.logger.debug('Local LLM response status=%s', getattr(resp, 'status_code', None))
                resp.raise_for_status()
                try:
                    return jsonify(resp.json()), 200
                except ValueError:
