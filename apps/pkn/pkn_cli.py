...
class PKNCLI:
    ...
    def print_header(self):
        ...
        print(
            f"{Colors.CYAN}{Colors.BOLD}  PKN Terminal CLI - Multi-Agent AI System{Colors.RESET}"
        )
        ...
        print(
            f"Commands: /help, /planning [on|off], /thinking [on|off], /status, /exit{Colors.RESET}\n"
        )
    ...
    def print_help(self):
        ...
        print(
            f"  {Colors.GREEN}/planning on|off{Colors.RESET}   Enable/disable planning mode for complex tasks"
        )
        print(
            f"  {Colors.GREEN}/thinking on|off{Colors.RESET}   Show/hide agent thinking process"
        )
        print(
            f"  {Colors.GREEN}/status{Colors.RESET}            Show agent and service status"
        )
        print(f"  {Colors.GREEN}/clear{Colors.RESET}             Clear screen")
        print(
            f"  {Colors.GREEN}/exit{Colors.RESET}, {Colors.GREEN}/quit{Colors.RESET}     Exit PKN CLI"
        )
        print(f"\n{Colors.CYAN}{Colors.BOLD}Agent Types:{Colors.RESET}")
        print(
            f"  {Colors.YELLOW}Coder{Colors.RESET}      - Code writing, debugging, refactoring"
        )
        print(
            f"  {Colors.YELLOW}Reasoner{Colors.RESET}   - Planning, logic, problem-solving"
        )
        print(
            f"  {Colors.YELLOW}Researcher{Colors.RESET} - Web research, documentation lookup"
        )
        print(
            f"  {Colors.YELLOW}Executor{Colors.RESET}   - System commands, file operations"
        )
        print(f"  {Colors.YELLOW}General{Colors.RESET}    - Quick Q&A, simple queries")
        print(
            f"  {Colors.YELLOW}Consultant{Colors.RESET} - External LLM for complex decisions"
        )
        print()
    ...
    async def show_status(self):
        ...
        print(
            f"  Planning Mode: {Colors.GREEN if self.planning_enabled else Colors.RED}{'Enabled' if self.planning_enabled else 'Disabled'}{Colors.RESET}"
        )
        print(
            f"  Show Thinking: {Colors.GREEN if self.show_thinking else Colors.RED}{'Yes' if self.show_thinking else 'No'}{Colors.RESET}"
        )
        ...
        for name, url in services.items():
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(
                        f"  {Colors.RED}✗{Colors.RESET} {name} (status: {response.status_code})"
                    )
            except:
        ...
        if self.agent_manager and hasattr(self.agent_manager, "agent_stats"):
            ...
            for agent, stats in self.agent_manager.agent_stats.items():
                print(
                    f"  {Colors.YELLOW}{agent}{Colors.RESET}: {stats['tasks_completed']} tasks, avg {stats['avg_time']:.1f}s"
                )
    ...
    async def run(self):
        ...
        while True:
            try:
                # Get user input
                user_input = input(
                    f"{Colors.GREEN}You{Colors.RESET} {Colors.GRAY}»{Colors.RESET} "
                ).strip()
                    ...
                    if cmd in ["exit", "quit", "q"]:
                        ...
                        if len(command) > 1:
                            if command[1] == "on":
                                self.planning_enabled = True
                                print(
                                    f"{Colors.GREEN}✓ Planning mode enabled{Colors.RESET}"
                                )
                            elif command[1] == "off":
                                self.planning_enabled = False
                                print(
                                    f"{Colors.YELLOW}⚠ Planning mode disabled{Colors.RESET}"
                                )
                        else:
                        ...
                        if len(command) > 1:
                            if command[1] == "on":
                                self.show_thinking = True
                                print(
                                    f"{Colors.GREEN}✓ Showing thinking process{Colors.RESET}"
                                )
                            elif command[1] == "off":
                                self.show_thinking = False
                                print(
                                    f"{Colors.YELLOW}⚠ Hiding thinking process{Colors.RESET}"
                                )
                        else:
                        ...
                        print(
                            f"Type {Colors.GREEN}/help{Colors.RESET} for available commands"
                        )
...
Examples:
  ...
  pkn-cli --project /path    Use custom project directory
    ...
    parser.add_argument(
        "--no-thinking", action="store_true", help="Hide agent thinking process"
    )
