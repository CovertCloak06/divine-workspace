...
def extract_function_body(lines, start_idx):
    ...
    while i < len(lines):
        ...
        if indent_level is not None and line.strip() and not line.startswith('#'):
            ...
            if current_indent < indent_level and not line.strip().startswith(('except', 'elif', 'else', 'finally')):
                break
...
def main():
    ...
    for group, route_list in routes.items():
        if group == 'other':
            print(f"⚠️  Skipping 'other' group - {len(route_list)} routes need manual classification")
            continue
        lines = generate_blueprint_file(group, route_list, imports, helpers, output_dir)
        total_lines += lines

    print(f"\n✅ Generated {len(routes) - 1} blueprint files")
    print(f"📏 Total lines: {total_lines} (from original {sum(1 for _ in server_file.read_text().splitlines())} lines)")
    print(f"✅ Average file size: {total_lines // (len(routes) - 1)} lines per blueprint")
