...
def fix_file_imports(file_path):
    ...
    content = re.sub(
        r"from agent_manager import", "from ..agents.manager import", content
    )
    content = re.sub(
    ...
    content = re.sub(
        r"from code_context import", "from ..memory.code_context import", content
    )
    content = re.sub(
        r"import code_context", "from ..memory import code_context", content
    )
    ...
    content = re.sub(
        r"import local_image_gen", "from ..image_gen import local_image_gen", content
    )

    # tools.* → ..tools.* (for routes and agents)
    content = re.sub(r"from tools\.(\w+) import", r"from ..tools.\1 import", content)
    content = re.sub(r"from tools import", "from ..tools import", content)
    ...
    # Config imports
    content = re.sub(
        r"from config\.settings import", "from ..config.settings import", content
    )
    ...
    if "agents/manager.py" in str(file_path):
        content = re.sub(
            r"^from tools import", "from ..tools import", content, flags=re.MULTILINE
        )
    ...
    if file_path.name == "server.py":
        content = re.sub(r"from routes import", "from .routes import", content)
    ...
    if "routes/" in str(file_path):
        ...
        if "app.logger" in content and "from flask import" in content:
            ...
            content = re.sub(r"\bapp\.logger\b", "current_app.logger", content)

        # Remove standalone 'app' references if current_app is used
        if "current_app" in content:
            content = re.sub(
                r"\bapp\.response_class\b", "current_app.response_class", content
            )
