...
def add_sequences_to_trie(seqs, trie):
    ...
    for seq, result in seqs:
        if not add_seq_to_trie(seq, result):
            dropped_sequences += 1
            warn(
                "Sequence collide: '%s' and '%s = %s'"
                % (existing_sequence_to_str(seq), "".join(seq), result)
            )
...
def check_for_warnings(tries):
    ...
    for c1 in string.ascii_uppercase:
        for c2 in string.ascii_uppercase:
            ...
            if r is not None:
                ll_warning = (
                    f" (but {seq_to_str(seq_ll)} = {r_ll} exists)"
                    if r_ll is not None
                    else ""
                )
                if r_l is None:
                    if r != r_ll:
                        warn(
                            f"has no lower case equivalent {seq_to_str(seq_l)}{ll_warning}",
                            seq=seq,
                            result=r,
                        )
                elif r != r_l:
                    warn(
                        f"is not the same as {seq_to_str(seq_l)} = {r_l}{ll_warning}",
                        seq=seq,
                        result=r,
                    )
...
print(
    "Compiled %d sequences into %d states. Dropped %d sequences. Generated %d warnings."
    % (total_sequences, len(automata), dropped_sequences, warning_count),
    file=sys.stderr,
)
