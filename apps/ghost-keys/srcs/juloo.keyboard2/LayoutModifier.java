...
public final class LayoutModifier
{
  ...
  static KeyboardData.Row[] macro_profiles;
  static String[] macro_profile_names;
  static int current_macro_profile = 0;
  ...
  public static KeyboardData modify_layout(KeyboardData kw)
  {
    ...
    // Add macro row at the very top (always show)
    if (macro_profiles != null && macro_profiles.length > 0)
      kw = kw.insert_row(macro_profiles[current_macro_profile], 0);
    return kw;
  }

  /** Switch to next macro profile */
  public static void nextMacroProfile()
  {
    if (macro_profiles != null && macro_profiles.length > 0)
      current_macro_profile = (current_macro_profile + 1) % macro_profiles.length;
  }

  /** Switch to previous macro profile */
  public static void prevMacroProfile()
  {
    if (macro_profiles != null && macro_profiles.length > 0)
      current_macro_profile = (current_macro_profile - 1 + macro_profiles.length) % macro_profiles.length;
  }

  /** Get current macro profile name */
  public static String getCurrentMacroProfileName()
  {
    if (macro_profile_names != null && macro_profile_names.length > current_macro_profile)
      return macro_profile_names[current_macro_profile];
    return "PKN";
  }
  ...
  public static void init(Config globalConfig_, Resources res)
  {
    ...
    try
    {
      ...
      // Load all macro profiles
      macro_profiles = new KeyboardData.Row[] {
        KeyboardData.load_row(res, R.xml.macro_row_pkn),
        KeyboardData.load_row(res, R.xml.macro_row_git),
        KeyboardData.load_row(res, R.xml.macro_row_sys),
        KeyboardData.load_row(res, R.xml.macro_row_docker),
        KeyboardData.load_row(res, R.xml.macro_row_nano),
      };
      macro_profile_names = new String[] { "PKN", "Git", "Sys", "Docker", "Nano" };
