  ...
  implements SharedPreferences.OnSharedPreferenceChangeListener
{
  ...
  private MacroDrawerView _macroDrawer = null;
  private boolean _macroDrawerShowing = false;
  ...
  public void onStartInputView(EditorInfo info, boolean restarting)
  {
    ...
    // Don't reset view if macro drawer is showing
    if (!_macroDrawerShowing)
      setInputView(_container_view);
  ...
  public class Receiver implements KeyEventHandler.IReceiver
  {
    public void handle_event_key(KeyValue.Event ev)
    {
      switch (ev)
      {
        ...
        case SWITCH_MACRO_NEXT:
          LayoutModifier.nextMacroProfile();
          setTextLayout(_config.get_current_layout());
          break;

        case SWITCH_MACRO_PREV:
          LayoutModifier.prevMacroProfile();
          setTextLayout(_config.get_current_layout());
          break;

        case OPEN_MACRO_DRAWER:
          showMacroDrawer();
          break;
  ...
  /** Show the macro drawer view */
  private void showMacroDrawer()
  {
    if (_macroDrawer == null)
    {
      _macroDrawer = new MacroDrawerView(this);
      _macroDrawer.setListener(new MacroDrawerView.OnMacroActionListener()
      {
        @Override
        public void onMacroTap(String command)
        {
          // Insert the macro command
          InputConnection conn = getCurrentInputConnection();
          if (conn != null)
            conn.commitText(command, 1);
          // Return to keyboard
          _macroDrawerShowing = false;
          setInputView(_container_view);
        }

        @Override
        public void onMacroLongPress(int index, String label)
        {
          // Show delete confirmation
          new android.app.AlertDialog.Builder(Keyboard2.this)
            .setTitle("Delete Macro")
            .setMessage("Delete \"" + label + "\"?")
            .setPositiveButton("Delete", (d, w) -> {
              _macroDrawer.getStorage().deleteMacro(index);
              _macroDrawer.refreshMacros();
            })
    ...
    _macroDrawerShowing = true;
    setInputView(_macroDrawer);
  }

  /** Show dialog to save current clipboard as macro */
  private void showSaveMacroDialog()
  {
    // Get text from clipboard to save as macro
    ...
    if (command.isEmpty())
    {
      android.widget.Toast.makeText(this,
          "Copy a command to clipboard first, then tap + to save it as a macro",
          android.widget.Toast.LENGTH_LONG).show();
      return;
    }

    final String cmdToSave = command;
    SaveMacroDialog dialog = new SaveMacroDialog(this, cmdToSave,
        new SaveMacroDialog.OnSaveListener()
    {
      @Override
      public void onSave(String label, String command)
      {
        MacroStorage storage = new MacroStorage(Keyboard2.this);
        storage.addMacro(label, command);
        android.widget.Toast.makeText(Keyboard2.this,
            "Macro \"" + label + "\" saved!",
            android.widget.Toast.LENGTH_SHORT).show();
        // Refresh drawer if it exists
        if (_macroDrawer != null)
          _macroDrawer.refreshMacros();
      }
