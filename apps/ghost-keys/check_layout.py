...
def missing_some_of(keys, symbols, class_name=None):
    ...
    if len(missing) > 0 and len(missing) != len(symbols):
        warn(
            "Layout includes some %s but not all, missing: %s"
...
def check_layout(layout):
    ...
    missing_required(
        keys, ["backspace", "delete"], "Layout doesn't define some important keys"
    )
    unexpected_keys(
        keys,
        [
    ...
    unexpected_keys(
        keys, filter(lambda k: k.strip() != k, keys), "Some keys contain whitespaces"
    )
    unexpected_keys(
        keys,
        ["f11_placeholder", "f12_placeholder"],
        "These keys are now added automatically",
    )
    ...
    if root.get("bottom_row") == "false":
        ...
        unexpected_keys(
            keys, bottom_row_keys, "Layout contains keys present in the bottom row"
        )
...
def parse_known_keys():
    ...
    with open("srcs/juloo.keyboard2/KeyValue.java", "r", encoding="utf-8") as f:
        known_keys = set((m.group(1) for m in re.finditer('case "([^"]+)":', f.read())))
